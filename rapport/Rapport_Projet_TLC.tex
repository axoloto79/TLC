...
\begin{center}
    ...
    \begin{tikzpicture}[scale=1.5, every node/.style={transform shape}]
        \draw[thick, navyblue, rounded corners=8pt] (0,0) rectangle (6,3);
        \node[navyblue] at (3,2.2) {\textbf{COMPILATION}};
        \node[darkteal, scale=0.8] at (3,1.5) {\texttt{Source.nno} $\rightarrow$ \texttt{Objet.co}};
        \node[charcoal, scale=0.7] at (3,0.8) {Analyseur + Machine Virtuelle};
    \end{tikzpicture}
    \vspace{2cm}
\end{center}
\thispagestyle{empty}
...
Chaque variable déclarée y est enregistrée avec des caractéristiques riches : son type (\texttt{integer} ou \texttt{boolean}), son rôle (\texttt{variable}, \texttt{parameter}, ou \texttt{local variable}), son adresse d'accès en mémoire ainsi que le mode du paramètre (\texttt{in} ou \texttt{in out}). 
...
\subsection{Gestion de l'Adressage Statique et Dynamique}
L'allocation d'adresse mémoire diffère fortement selon la portée :
\begin{itemize}
    \item \textbf{Global} : L'adresse allouée correspond à l'indice direct dans le bloc d'activation principal globale de la pile virtuelle (adressage direct absolu de 0 à $N$).
    \item \textbf{Local / Paramètre} : L'adresse est calculée comme un décalage relatif par rapport au registre de base (\texttt{self.base}) du bloc de liaison créé lors de l'appel. Les paramètres et les variables locales sont stockés de manière contiguë.
\end{itemize}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \draw[dashed, charcoal] ($(top.north west)+(-0.5,0.2)$) -- ($(prev.south west)+(-0.5,-0.2)$);
        \draw[dashed, charcoal] ($(top.north east)+(0.5,0.2)$) -- ($(prev.south east)+(0.5,-0.2)$);
    \end{tikzpicture}
